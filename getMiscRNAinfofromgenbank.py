...
# What it does
#     it searches the 'features' in a GenBank file for the 'misc_RNA' feature.
#     Once a 'misc_RNA' feature is found, it looks for the required information
#     It also looks for the misc_RNA location and gets the actual misc_RNA from the whole
...
if len(sys.argv) != 3:
    print "Usage: getMiscRNAinfofromgenbank.py <inputfile> <outputfile>"
    quit()
...
# print the start
print "Getting the Misc RNA information and appending to the outputfile..."
...
for featur in gb_record.features:
    if featur.type == 'misc_RNA':
        ...
        if featur.qualifiers.has_key('note'):
            notename = featur.qualifiers['note']
        else:
                        notename = ''

        if featur.qualifiers.has_key('product'):
            productnote = featur.qualifiers['product']
        else:
                        productnote = ''
        ...
        # should probably use the CSV module here
                print >> outputfilehandle, '%s~%s~%s~%s~%s~%s' % (notename, productnote, genelocustag, start, end, sense)
